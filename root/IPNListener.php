...
if ($result != 'fail') 
{
	...
	switch (strtoupper($p->fields['txn_type']))
	{
		...
		case 'RECURRING_PAYMENT_PROFILE_CREATED':
			...
			$vars_array['portal']			= 'paypal'; 
			$vars_array['subscriber_id']	= $p->subscriber_id;
		...
		case 'RECURRING_PAYMENT_PROFILE_CANCEL': 			// Turn off as subscriber
			list($groupid, $userid) = explode('-', $p->fields['rp_invoice_id']);
			if (cancel_recurring_payment($p->fields['recurring_payment_id'], $params['i']))
			{
				$sql = 'UPDATE ' . MEMBERSHIP_TABLE . " SET subscriber_id='', portal='' WHERE user_id='{$userid}'";
				$db->sql_query($sql);	
			 	$p->write_results("Paypal Subscription cancelled for user-group {$userid} - {$groupid}");
			}
			else
			{
		...
		case 'RECURRING_PAYMENT':
			if ($p->fields['payment_status'] == 'Completed')
			{
				...
				$vars_array['txn_id'] = $p->fields['txn_id'];
...
//			$db->sql_query($sql);		
		...
		case 'EXPRESS_CHECKOUT':
			...
			{
				list($groupid, $userid) = explode('-', $p->fields['rp_invoice_id']);
				process_payment($userid, false);//, (strtotime($p->fields['NEXTBILLINGDATE'])));//TODO:Check the nextbilling date is ok for express checkout
				$vars_array['portal'] = 'paypal';
			}
		break;
