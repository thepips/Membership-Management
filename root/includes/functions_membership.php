...
{
	global $db, $config;
	$sql = 'UPDATE ' . MEMBERSHIP_TABLE . ' SET uncleared = 0, datepaid=' . 
		time() . ($renewal_date=='' ? '' : ', renewal_date = '. $renewal_date) . " WHERE user_id ={$userid} AND group_id={$groupid}";
 	$db->sql_query($sql);
...
function process_payment($groupid, $userid, $uncleared)
{
	...
	else
	{
        ...
        // New member
		group_user_add($groupid,$userid,null,null,null,null,$config['ms_approval_required']);
...
function display_subscription_message($userid,$groupid, $type='')
{
    ...
    $result=(array(
        ...
        'RENEWAL_ACTION'    => append_sid("{$phpbb_root_path}application.$phpEx","mode=apply&result=index&i={$userid}&g={$groupid}"),
        'MEMBERSHIP_NO'     => $row['membership_no'],                        
		...
		'S_ACTION'          => append_sid("{$phpbb_root_path}application.$phpEx","mode=associate&action=validate&member={$is_member}&i={$userid}&g={$groupid}&ref={$row['membership_no']}"),
	));
...
global $db, $user, $config;
    ...
    $sql_array = array(
    	...
    	'LEFT_JOIN' => array(
    		array(
    			'FROM'  => array(MEMBERSHIP_TABLE => 'm'),
    			'ON'    => 'u.user_id = m.user_id AND m.group_id=' . $config['ms_subscription_group']
    		),
    		array(
    ...
    $sql_array['SELECT'] = 'u.user_id, user_colour, username, user_regdate, user_lastvisit, user_posts, m.reminderdate, m.remindercount, pfd.pf_ms_realname, m.renewal_date';
...
function process_associate($userid, $associate_id)
{
    ...
    if ($associate_id>0)
    {
        ...
        $result =$db->sql_query($sql);
        $pending = $db->sql_fetchfield('user_pending');
		group_user_add($groupid,$associate_id,null,null,null,null,$pending);
    }
    // 6. update membership record
    $sql = 'UPDATE ' . MEMBERSHIP_TABLE . " SET associate_id={$associate_id} WHERE user_id=$userid AND group_id={$groupid}";
    $result =$db->sql_query($sql);
}
